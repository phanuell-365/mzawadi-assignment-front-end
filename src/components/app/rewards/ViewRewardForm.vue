...
interface ViewRewardFormProps {
  rewardId: string;
}

const props = defineProps<ViewRewardFormProps>();

const rewardsStore = useRewardsStore();

const reward: Ref<RewardObjectWithConsumerAndProduct | undefined> = ref();

const rewardObj = ref({});

try {
  reward.value = await rewardsStore.fetchRewardByIdWithProductAndDistributor(
    props.rewardId
  );

  rewardObj.value = Object.fromEntries(
    Object.entries(reward.value).filter(([key]) => {
      return key !== "id";
    })
  );
} catch (error: any) {
