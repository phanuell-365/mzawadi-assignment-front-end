...
const props = defineProps<ViewPointFormProps>();

const pointsStore = usePointsStore();

const point: Ref<PointObjectWithConsumerProductAndDistributor | undefined> =
  ref();

const pointObj = ref({});

try {
  point.value = await pointsStore.fetchPointWithConsumerAndDistributorDataById(
    props.pointId
  );

  pointObj.value = Object.fromEntries(
    Object.entries(point.value).filter(([key]) => {
      return key !== "id";
    })
  );
} catch (error: any) {
